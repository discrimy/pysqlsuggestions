...
- **Which version goes first.** `v0.6.0` is tagged and `pyproject.toml` reads
  `0.6.0`, so a release cut from that tag passes the guard and publishes today.
  But `CHANGELOG.md` holds an Unreleased section with the `CREATE TABLE` work,
  so `0.7.0` may be the more honest first upload. A release decision, not a
  workflow one.
